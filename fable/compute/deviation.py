"""Pathway-vs-baseline deviation maths.

Moved verbatim from the legacy ``src/comparison.py`` so the bundle builder and
the golden tests share exactly one implementation. The legacy ``src/`` copy is
kept only until the Streamlit dashboard is retired.
"""
